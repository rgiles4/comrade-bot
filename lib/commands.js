...
export const commands = [
    ...
    { // Upvote a user
        ...
        async execute(interaction) {
            ...
            const { onCooldown, remaining } = utilities.getCooldown(interaction.name, interaction.user.id);
            /*  1. Check if on cooldown FIRST
                2. Then check if self-upvote
                3. Then check if bot-upvote
                4. Finally, process the upvote */
            const cooldownTimeMins = Number(Math.ceil(remaining / 60));
            if (onCooldown) {
                console.log(`User ${interaction.user.id} attempted to upvote while on cooldown.`);
                await interaction.reply(`You can only upvote once every 30 minutes. Your cooldown is ${cooldownTimeMins} minutes.`);
                return;
    ...
    { // Downvote a user
        ...
        async execute(interaction) {
            ...
            if (onCooldown) {
                ...
                await interaction.reply(`You can only downvote once every 30 minutes. Your cooldown is ${cooldownTimeMins} minutes.`);
                return;
            } else if (targetUser.id === interaction.user.id) {
